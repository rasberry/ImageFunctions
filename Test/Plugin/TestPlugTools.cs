...
public class TestPlugTools
{
	...
	public void BeGreaterThanZero(int value, bool includeZero, Result result)
	{
		...
		var tir = PlugTools.BeGreaterThanZero(ti,includeZero);
		...
		var tlr = PlugTools.BeGreaterThanZero(tl,includeZero);
		...
		var tdr = PlugTools.BeGreaterThanZero(td,includeZero);
	...
	public void BeBetween(int value, int low, int high, bool lI, bool hI, Result result)
	{
		...
		var tir = PlugTools.BeBetween(ti,low,high,lI,hI);
		Assert.AreEqual(result,tir.Result);
	}
}